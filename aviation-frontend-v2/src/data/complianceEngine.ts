...
function checkAircraftCompliance(
  ...
  const aircraftCategories =
    categoryMapping[aircraft.categoryLabel || aircraft.category] || [];
...
function checkCertificationCompliance(
  ...
  if (aircraft.model.includes("E2")) {
    ...
    findings.push(
      "Proteção avançada contra gelo e condições meteorológicas adversas",
    );
  }
  // E-Jets primeira geração
  else if (
    aircraft.model.includes("E170") ||
    aircraft.model.includes("E175") ||
    aircraft.model.includes("E190") ||
    aircraft.model.includes("E195")
  ) {
    ...
    recommendations.push(
      "Considerar atualizações de aviônicos para manter conformidade com requisitos modernos",
    );
    complianceScore -= 5;
  }
  ...
  // Aeronaves militares
    ...
    aircraft.model.includes("KC-390") ||
    aircraft.model.includes("C-390")
  ) {
    findings.push(
      `${aircraft.model} certificado para operações militares e civis`,
    );
    findings.push("Dupla certificação (militar e civil)");
    findings.push(
      "Conformidade com requisitos especiais de carga e operação tática",
    );
  }
  // A-29 Super Tucano
    ...
    findings.push(
      "Certificação de acordo com requisitos de aeronave de treinamento e ataque leve",
    );
    complianceScore -= 10; // Não tem certificação civil completa
    recommendations.push(
      "Certificação militar específica, não aplicável operação civil",
    );
  }
  // EMB-202 Ipanema
    ...
    findings.push(
      "Certificação especial para aplicação de defensivos agrícolas",
    );
  ...
  if (
    regulation.reference.includes("25") ||
    regulation.reference.includes("RBAC 25") ||
    regulation.reference.includes("CS-25")
  ) {
    if (aircraft.categoryLabel === "Jatos Comerciais") {
      findings.push(
        "Estrutura certificada para cargas limite com fator de segurança 1.5",
      );
...
function checkOperationCompliance(
  ...
  if (
    regulation.reference.includes("121") ||
    regulation.reference.includes("RBAC 121")
  ) {
    if (aircraft.categoryLabel === "Jatos Comerciais") {
      ...
      if (aircraft.model.includes("E2")) {
        ...
        recommendations.push(
          "Considerar upgrade de aviônicos para conformidade FANS/CPDLC",
        );
        complianceScore -= 5;
      }
    }
  ...
  if (
    regulation.reference.includes("135") ||
    regulation.reference.includes("RBAC 135")
  ) {
  ...
  if (
    regulation.reference.includes("91") ||
    regulation.reference.includes("RBAC 91")
  ) {
  ...
  if (
    regulation.reference.includes("137") ||
    regulation.reference.includes("RBAC 137")
  ) {
...
function checkMaintenanceCompliance(
  ...
  if (aircraft.model.includes("E2")) {
    ...
    findings.push(
      "Programa de manutenção estabelecido com histórico comprovado",
    );
    recommendations.push(
      "Considerar programa de modernização para reduzir custos de manutenção",
    );
    complianceScore -= 5;
  }
  ...
  if (
    regulation.reference.includes("145") ||
    regulation.reference.includes("Part-145") ||
    regulation.reference.includes("RBAC 145")
  ) {
    findings.push(
      "Embraer Service Centers certificados Part 145/RBAC 145 globalmente",
    );
    findings.push("Rede de MROs autorizados em todos os continentes");
    recommendations.push(
      "Utilizar apenas centros de manutenção certificados Embraer",
    );
  }
  ...
  recommendations.push(
    "Manter todos os registros de manutenção atualizados e acessíveis",
  );
  recommendations.push(
    "Seguir rigorosamente as Diretrizes de Aeronavegabilidade (ADs)",
  );
...
function checkSafetyCompliance(
  ...
  // TCAS/ACAS
  if (
    regulation.reference.includes("TCAS") ||
    regulation.reference.includes("ACAS")
  ) {
    if (aircraft.categoryLabel === "Jatos Comerciais") {
      ...
      if (
        aircraft.model.includes("Praetor") ||
        aircraft.model.includes("Phenom 300")
      ) {
        ...
        recommendations.push(
          "Considerar upgrade para TCAS II para operações em espaço aéreo classe A",
        );
        complianceScore -= 10;
      }
    }
  ...
  // GPWS/EGPWS
  if (
    regulation.reference.includes("GPWS") ||
    regulation.reference.includes("EGPWS")
  ) {
    if (aircraft.categoryLabel === "Jatos Comerciais") {
  ...
  if (
    regulation.reference.includes("SMS") ||
    regulation.reference.includes("SGSO")
  ) {
    ...
    recommendations.push(
      "Implementar programa robusto de SMS conforme regulamentação",
    );
    recommendations.push(
      "Estabelecer cultura de segurança proativa na organização",
    );
  ...
  // Gravadores de voo
  if (
    regulation.reference.includes("FDR") ||
    regulation.reference.includes("CVR")
  ) {
    if (aircraft.categoryLabel === "Jatos Comerciais") {
...
function checkEnvironmentalCompliance(
  ...
  // E-Jets primeira geração
    ...
    recommendations.push(
      "Considerar retrofit de winglets para melhorar eficiência",
    );
    recommendations.push(
      "Avaliar programa de modernização para E2 para ganhos ambientais significativos",
    );
    complianceScore -= 15;
  }
  // Jatos executivos
    ...
    if (aircraft.model.includes("Praetor")) {
      ...
      findings.push(
        "Alcance estendido reduz necessidade de paradas intermediárias",
      );
    }
  ...
  // KC-390
    ...
    findings.push(
      "Certificação para operações em aeroportos com restrições de ruído",
    );
  ...
  recommendations.push(
    "Implementar procedimentos de operação para minimizar impacto ambiental",
  );
  recommendations.push(
    "Utilizar combustível sustentável de aviação (SAF) quando disponível",
  );
  recommendations.push(
    "Otimizar perfis de voo para reduzir consumo e emissões",
  );
...
function checkSecurityCompliance(
  ...
  if (aircraft.categoryLabel === "Jatos Comerciais") {
    findings.push(
      "Porta de cockpit reforçada conforme regulamentação pós-9/11",
    );
    ...
    findings.push(
      "Possibilidade de instalação de sistemas de segurança customizados",
    );
  }

  recommendations.push(
    "Seguir todos os procedimentos de segurança da autoridade local",
  );
  recommendations.push(
    "Treinar tripulação em procedimentos de ameaças e interferência ilícita",
  );
...
export function generateComplianceReport(
  ...
  const applicableRegulations = regulations.filter(
  ...
  const partialChecks = checks.filter(
    (c) => c.status === "PARTIAL_COMPLIANCE",
  ).length;
  const criticalIssues = checks.filter(
    (c) => c.severity === "CRITICAL" && c.status !== "COMPLIANT",
  ).length;

  const totalApplicable = checks.filter(
    (c) => c.status !== "NOT_APPLICABLE",
  ).length;
  const compliancePercentage =
    totalApplicable > 0 ? (compliantChecks / totalApplicable) * 100 : 100;
  ...
  if (criticalIssues > 0) {
    generalRecommendations.push(
      "Resolver imediatamente os problemas críticos identificados antes de operações",
    );
  }

  if (nonCompliantChecks > 0) {
    generalRecommendations.push(
      "Desenvolver plano de ação para abordar não conformidades",
    );
    generalRecommendations.push(
      "Consultar com autoridade reguladora sobre requisitos específicos",
    );
  }
  ...
  generalRecommendations.push(
    "Manter documentação atualizada de todos os certificados e aprovações",
  );
  generalRecommendations.push(
    "Estabelecer programa de monitoramento contínuo de conformidade",
  );
  generalRecommendations.push(
    "Participar de programas de atualização regulatória do fabricante",
  );
  ...
  const aircraftInfo: AircraftInfo = {
    ...
    max_seats: aircraft.specs?.capacity || "N/A",
    max_weight_kg: 0,
  };
